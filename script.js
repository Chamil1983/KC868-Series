...
function initHTSensorsUI() {
    ...
    // Add HT sensors configuration button to the analog-inputs page
...
function createHTSensorConfigModal() {
    ...
    // FIX: Setup form submission handler properly
    const form = document.getElementById('ht-sensor-config-form');
    if (form) {
        form.addEventListener('submit', function(e) {
            e.preventDefault(); // This is critical - prevent the default form submission
            console.log('HT sensor config form submitted');
            saveHTSensorConfig();
        });
    } else {
        console.error('HT sensor config form not found');
    }
...
// Replace the saveHTSensorConfig function with this improved version
function saveHTSensorConfig() {
    ...
    // Save each sensor configuration sequentially for better reliability
    let savePromiseChain = Promise.resolve();
    
    configs.forEach(config => {
        savePromiseChain = savePromiseChain.then(() => {
            console.log(`Saving HT${config.index + 1} as type ${config.sensorType}`);
            ...
            .then(data => {
                console.log(`Response for HT${config.index + 1}:`, data);
                if (data.status !== 'success') {
        ...
        .catch(error => {
            console.error(`Error saving HT${config.index + 1} configuration:`, error);
            return { status: 'error', message: error.message };
        });
    });
    
    // After all saves are attempted
    savePromiseChain.then(() => {
        // Hide the modal
        const modal = document.getElementById('ht-sensor-config-modal');
        if (modal) {
            modal.style.display = 'none';
    ...
    .catch(error => {
        console.error('Error during sensor configuration:', error);
