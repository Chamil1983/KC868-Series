...
void resetEthernet() {
    // Software reset for Ethernet module through ESP32 register manipulation
    // This approach doesn't need a physical connection to a GPIO pin
...
// Enhanced Ethernet initialization - software-only fix
void initEthernet() {
    ...
    debugPrintln("Starting Ethernet initialization...");
    ...
    delay(1000);
    if (!ETH.linkUp()) {
        debugPrintln("Trying alternative PHY address 0...");
        ETH.begin(ETH_PHY_LAN8720, 0, ETH_PHY_MDC, ETH_PHY_MDIO, ETH_PHY_POWER, ETH_CLOCK_GPIO17_OUT);
    }

    delay(1000);
    if (!ETH.linkUp()) {
        debugPrintln("Trying alternative PHY address 1...");
    ...
    debugPrintln("Waiting for Ethernet link...");
    int ethAttempts = 0;
    ...
    if (ETH.linkUp()) {
        ...
        debugPrintln("Ethernet link is DOWN. Check cable connection or LAN8720 initialization.");
        wiredMode = false;
        ethConnected = false;
    }
}
...
// Handler to update HT sensor configuration
